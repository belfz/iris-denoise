...
use image_dwt::{kernels::B3SplineKernel, layer::{WaveletLayer, WaveletLayerBuffer}, recompose::{OutputLayer, RecomposableWaveletLayers}, transform::ATrousTransform};

pub fn denoise_image_experimental_with_a_trous(image: DynamicImage) -> DynamicImage {
    let a_trous = ATrousTransform::new(&image, 9, B3SplineKernel);
    println!("a trous!");
                ...
                match buffer {
                    WaveletLayerBuffer::Grayscale { data } => {
                        for (x, y, pixel) in new_buffer.enumerate_pixels_mut() {
                            *pixel = Luma([(data[[y as usize, x as usize]] * u16::MAX as f32) as u16])
                        }
                    ...
                    WaveletLayerBuffer::Rgb { data } => {
                        for (x, y, pixel) in new_buffer.enumerate_pixels_mut() {
                            *pixel = Luma([(data[[y as usize, x as usize, 0]] * u16::MAX as f32) as u16])
                        }
                        let mut image = DynamicImage::ImageLuma16(new_buffer).to_luma8();
                        // Bilateral filter is a de-noising filter. Apply it to the image.
                        image = imageproc::filter::bilateral_filter(&image, 10, 10., 3.);
